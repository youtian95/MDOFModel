...
if __name__ == '__main__':
    ...
    print("====== 开始运行 GeneralModelWrapper 的 3D IDA 分析 ======")
    ...
    # 倒塌分析：自动识别 3D CSV，MLE 拟合倒塌易损性 + 筛选非倒塌记录
    ca = Collapse.CollapseAnalysis(CFDir/'IDA_results.csv', building_type='S1L')
    collapse_result = ca.fit_collapse_fragility(fig_path=CFDir/'collapse_fragility.jpg')
    print(f"倒塌易损性中值 Sa (g): {collapse_result['median']:.3f} g")
